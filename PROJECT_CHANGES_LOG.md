...
## 18. Ingestion refactor: split into extract, transform, load, plus a --live flag
**Change:** Split the ETL pipeline out of the single ingestion script into `extract.py`, `transform.py`, and `load.py`, orchestrated by `update_stock_data.py`. Added an `argparse` `--live` flag so the live price pipeline can optionally start after the batch load, controlled from the command line rather than needing a separate manual step. Switched the companies insert from a row count guard to a proper upsert (`INSERT ... ON DUPLICATE KEY UPDATE`).
**Why:** Separating extract, transform, and load into their own functions and files makes each stage independently testable and readable, and isolates failures to a specific stage rather than one long script. The `--live` flag lets one command drive both the batch and live pipelines instead of running them separately by hand. The upsert is more robust than the row count check, since it updates existing company rows if their info changes instead of just skipping the insert once three rows exist.

Two bugs came up during this refactor and are worth recording. First, `live_price_updater.py`'s `engine` was originally built inside a `sql_connection()` function and never escaped that function's local scope, so `live_price_handler` and `periodic_pull` couldn't see it. Fixed by declaring `engine = None` at module level and using `global engine` inside `sql_connection()` so the function writes to the module-level variable instead of a local one. Second, `load_to_sql()` had no explicit `return` on success, so it implicitly returned `None` whether it succeeded or hit its `except` block, meaning `update_stock_data.py`'s success/failure check could never actually tell the difference. Fixed by adding `return True` after the commit, and correcting the check in `update_stock_data.py` to `if result is True:`.
